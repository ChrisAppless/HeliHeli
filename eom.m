...
mu       = (V_true / (Omega * R)) * cos(alpha_c);
lambda_c = (V_true / (Omega * R)) * sin(alpha_c);
VOR      = V_true / (Omega * R);

if V_true <= 0.01, mu = 0; lambda_c = 0; VOR = 0; end
...
for iter = 1:100
    ...
    if abs(li_new - li) < 1e-6, break; end
    li = 0.5*li + 0.5*li_new;
end
...
D  = Afront * fuse_CD * 0.5 * rho * V_true^2;
